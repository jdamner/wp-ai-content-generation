...
class Model { 
	...
	public function __construct(
		...
		$this->content          = is_string( get_transient( self::PREFIX . $this->id . '_content' ) ) ? 
			get_transient( self::PREFIX . $this->id . '_content' ) : null;
		$this->content_analysis = is_object( get_transient( self::PREFIX . $this->id . '_content_analysis' ) ) ?
			get_transient( self::PREFIX . $this->id . '_content_analysis' ) : null;
		$this->intent_analysis  = is_object( get_transient( self::PREFIX . $this->id . '_intent_analysis' ) ) ? 
			get_transient( self::PREFIX . $this->id . '_intent_analysis' ) : null;
		$this->components       = is_array( get_transient( self::PREFIX . $this->id . '_components' ) ) ?
			get_transient( self::PREFIX . $this->id . '_components' ) : null;
	}
	...
	public function generate_components(): void {
		$schema   = (object) json_decode( file_get_contents( __DIR__ . '/schema/ui.schema.json' ) ?: '{}' ); // phpcs:ignore Universal.Operators.DisallowShortTernary.Found
		$messages = array(
			array(
				'role'    => 'system',
	 ...
	 * 
	 * @return object An object containing the content from the AI response.
	 * 
	...
	private function get_content_from_response( \OpenAI\Responses\Chat\CreateResponse $response ): object {
		...
		if ( ! is_object( $content ) ) {
			throw new \Exception( 'Invalid content format received from AI service.' );
		}
